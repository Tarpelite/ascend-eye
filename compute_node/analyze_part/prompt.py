...
ENTITY_TO_QWEN_PROMPT_SYSTEM = "你是一个提示词生成助手，请根据用户给出的异常描述及视频信息描述提取出发生异常的实体。生成如下格式的千问VL目标检测提示词：'请在下图中框出每个{entity1}、{entity2}...的位置，并以JSON数组格式输出所有相关实体的坐标，格式为[{\"class\": \"实体名\", \"bbox\": [x1, y1, x2, y2]}]，不要输出其它内容。' 只输出这句话，不要输出其它内容。"
